...
public class JsonValue implements Cloneable, Iterable<JsonValue> {
     ...
     * (and value) is specified by a transformation function. It is up to the
     ...
     * resemblesâ€”but is not guaranteed to conform toâ€”JSON syntax. This method
     ...
     *
     * @return A Stream of JsonValue-wrapped Objects.
     */
    public Stream<JsonValue> stream() {
        if (isList()) {
            List<Object> list = asList();
            return IntStream.range(0, list.size()).mapToObj(i -> new JsonValue(list.get(i), pointer.child(i)));
        } else if (isCollection()) {
            throw new UnsupportedOperationException("Non-List Collections cannot be streamed");
        } else if (isMap()) {
            return asMap().entrySet().stream().map(e -> new JsonValue(e.getValue(), pointer.child(e.getKey())));
        } else {
