...
public class JsonPointer implements Iterable<String> {
     ...
     * The returned JSON pointer will have been formed of the concatenation of
     * this JSON pointer and the provided JSON pointer.
     *
     * @param suffix the JSON pointer to be appended to this JSON pointer.
     * @return A new JSON pointer which is a descendant of this JSON pointer.
     */
    public JsonPointer concat(final JsonPointer suffix) {
        if (isEmpty()) {
            return suffix;
        } else if (suffix.isEmpty()) {
            return this;
        } else {
            String[] newTokens = new String[tokens.length + suffix.tokens.length];
            System.arraycopy(tokens, 0, newTokens, 0, tokens.length);
            System.arraycopy(suffix.tokens, 0, newTokens, tokens.length, suffix.tokens.length);
