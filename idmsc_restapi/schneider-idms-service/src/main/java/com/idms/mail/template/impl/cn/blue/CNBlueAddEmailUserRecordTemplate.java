...
public class CNBlueAddEmailUserRecordTemplate extends CNBlueDefaultTemplate {

	public CNBlueAddEmailUserRecordTemplate(EmailTemplateInput input) {
		super(input);
	}
...
//		String filePath = _instance.getConfiguration("user.add.email.template.blue.cn");
//		EmailTemplate template = new EmailTemplate();
//		template.setEmailTemplatePath(filePath);
//		return template;
		
		String filePath = input.getConfiguration().getIDMS_USER_ADD_EMAILTEMPLATE_CN();
		EmailTemplate template = new EmailTemplate();
		template.setEmailTemplatePath(filePath);
		return template;
	}
