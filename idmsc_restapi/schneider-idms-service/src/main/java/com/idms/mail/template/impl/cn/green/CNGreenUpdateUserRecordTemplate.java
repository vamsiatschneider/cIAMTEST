...
public class CNGreenUpdateUserRecordTemplate extends CNGreenDefaultTemplate {

	public CNGreenUpdateUserRecordTemplate(EmailTemplateInput input) {
		super(input);
	}
...
//		String filePath = _instance.getConfiguration("user.update.email.template.cn");
//		EmailTemplate template = new EmailTemplate();
//		template.setEmailTemplatePath(filePath);
//		return template;
		
		String filePath = input.getConfiguration().getIDMS_USER_UPDATE_EMAILTEMPLATE_CN();
		EmailTemplate template = new EmailTemplate();
		template.setEmailTemplatePath(filePath);
		return template;
	}
