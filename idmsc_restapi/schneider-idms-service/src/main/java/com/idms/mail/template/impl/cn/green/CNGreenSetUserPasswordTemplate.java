...
public class CNGreenSetUserPasswordTemplate extends CNGreenDefaultTemplate {

	public CNGreenSetUserPasswordTemplate(EmailTemplateInput input) {
		super(input);
	}
...
//		String filePath = _instance.getConfiguration("user.reset.password.email.template.cn");
//		EmailTemplate template = new EmailTemplate();
//		template.setEmailTemplatePath(filePath);
//		return template;
		
		String filePath = input.getConfiguration().getIDMS_USER_REST_PASSWORD_EMAILTEMPLATE_CN();
		EmailTemplate template = new EmailTemplate();
		template.setEmailTemplatePath(filePath);
		return template;
	}
