...
public class ENGreenUserRegistrationTemplate extends ENGreenDefaultTemplate {

	public ENGreenUserRegistrationTemplate(EmailTemplateInput input) {
		super(input);
	}
...
//		String filePath = _instance.getConfiguration("user.registration.withpwd.email.template.en");
//		EmailTemplate template = new EmailTemplate();
//		template.setEmailTemplatePath(filePath);
//		return template;
		
		String filePath = input.getConfiguration().getIDMS_USER_REGESTRATION_WITHPWD_EMAILTEMPLATE_EN();
		EmailTemplate template = new EmailTemplate();
		template.setEmailTemplatePath(filePath);
		return template;
	}
