...
public class CNGreenInternalRegistrationTemplate extends CNGreenDefaultTemplate {

	public CNGreenInternalRegistrationTemplate(EmailTemplateInput input) {
		super(input);
	}
...
//		String filePath = _instance.getConfiguration("prm.internal.registration.email.template.cn");
//		EmailTemplate template = new EmailTemplate();
//		template.setEmailTemplatePath(filePath);
//		return template;
		
		String filePath = input.getConfiguration().getPRM_INTERNAL_USER_REGESTRATION_EMAILTEMPLATE_CN();
		EmailTemplate template = new EmailTemplate();
		template.setEmailTemplatePath(filePath);
		return template;
	}
