...
public class DynamicUserRegistrationOTPTemplate {

	private DynamicEmailTemplateInput input;
	
	public DynamicUserRegistrationOTPTemplate(DynamicEmailTemplateInput input) {
		this.input = input;
	}
...
//		String filePath = _instance.getConfiguration("user.registration.withpwd.otp.email.template");
//		EmailTemplate template = new EmailTemplate();
//		template.setEmailTemplatePath(filePath);
//		return template;
		
		String filePath = input.getConfiguration().getIDMS_USER_REGISTRATION_WITHPWD_OTP_EMAILTEMPLATE();
